...
function PriceBadge(props){ return <span className={'px-2 py-1 rounded-full text-xs border font-medium ' + (toneMap[props.tone] || toneMap.gray)}>{props.children}</span>; }
  ...
  var dataRows = filtered.map(function(p){
    var dec = priceDecisions[p.id];
    return [p.pr.issue, p.oem, p.jss, p.customerPart, p.desc, p.pr.price != null ? p.pr.price : '', p.pr.cost != null ? p.pr.cost : '', p.pr.margin != null ? Math.round(p.pr.margin*100)+'%' : '—', p.pr.target != null ? p.pr.target : '', priceProposal[p.id] !== undefined ? priceProposal[p.id] : '', dec ? dec.status : '', dec ? dec.gradedYear : CURRENT_YEAR];
  });
  var rows = [cols.map(function(c){ return c.header; })].concat(dataRows);
  var ws = XLSX.utils.aoa_to_sheet(rows);
  var wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, 'Price Review');
  XLSX.writeFile(wb, 'Price_Review_' + (priceFilter !== 'All' ? priceFilter.replace(/\s/g,'_')+'_' : '') + new Date().toISOString().slice(0,10) + '.xlsx');
...
{['All','Missing Price','Below Cost','Thin Margin','Healthy','Flagged'].map(function(f){
  ...
  }} style={{marginLeft:'auto',fontSize:'0.75rem',color:'#6b7280',whiteSpace:'nowrap',background:'none',border:'none',cursor:'pointer',padding:'0.25rem 0.5rem'}} onMouseEnter={function(e){e.target.style.color='#111827';}} onMouseLeave={function(e){e.target.style.color='#6b7280';}}>↑ Jump to top</button></div>
