...
function PhasePill(props){ var m = phaseMeta[props.phase]; return <span className={'px-2 py-1 rounded-full text-xs border font-medium ' + (tonePill[m.tone] || tonePill.gray)}>{m.label}</span>; }
  ...
  function styleHdr(row){ row.eachCell(function(c){ c.font={bold:true,name:'Aptos Narrow',size:11}; c.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FFCAEDFB'}}; c.alignment={horizontal:'center',vertical:'middle'}; }); }
  var ws1=wb.addWorksheet('Phase Summary');
  ws1.columns=[{width:20},{width:14},{width:12},{width:34}];
  styleHdr(ws1.addRow(['Phase','Part Count','% of Total','Description']));
  var tot=phased.length;
  order.forEach(function(k){ var m=phaseMeta[k]; var cnt=phased.filter(function(p){return p.phase===k;}).length; ws1.addRow([m.label,cnt,tot>0?(cnt/tot*100).toFixed(1)+'%':'0%',m.note]); });
  var ws2=wb.addWorksheet('Per-OEM Breakdown');
  ws2.columns=[{width:18},{width:12},{width:12},{width:12},{width:12},{width:14},{width:12},{width:12}];
  styleHdr(ws2.addRow(['OEM','0–5 Yrs','5–10 Yrs','10–15 Yrs','15+ Yrs','Unknown Age','Total','Cleanup %']));
  oemPhase.forEach(function(o){ ws2.addRow([o.oem,o.p1,o.p2,o.p3,o.p4,o.unknown,o.total,o.cleanupPct+'%']); });
  var ws3=wb.addWorksheet('Part Detail');
  ws3.columns=[{width:16},{width:12},{width:16},{width:18},{width:40},{width:14},{width:14}];
  styleHdr(ws3.addRow(['Phase','Years Left','OEM','JSS Part','Description','Service EOP','Status']));
  phased.forEach(function(p){ var m=phaseMeta[p.phase]; ws3.addRow([m.label,p.yearsLeft===null?'Unknown':p.yearsLeft<=0?'Overdue':p.yearsLeft,p.oem,p.jss,p.desc,p.serviceEop,p.active]); });
  [ws1,ws2,ws3].forEach(function(ws){ ws.views=[{state:'frozen',ySplit:1}]; ws.autoFilter={from:{row:1,column:1},to:{row:1,column:ws.columnCount}}; });
  var buf=await wb.xlsx.writeBuffer();
  var blob=new Blob([buf],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'});
  var url=URL.createObjectURL(blob); var a=document.createElement('a'); a.href=url; a.download='service-life-phase-report-'+CURRENT_YEAR+'.xlsx'; a.click(); URL.revokeObjectURL(url);
