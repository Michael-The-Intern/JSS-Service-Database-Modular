...
function ReportsExports() {
  ...
  const { page, setPage, parts, rawParts, setRawParts, rawAudit, setRawAudit, _supaWrite,
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-5"><div className="bg-white rounded-xl border border-gray-200 overflow-hidden"><div className="p-4 border-b border-gray-200 font-bold text-gray-900">Recent Exports</div><div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-gray-50 text-gray-600"><tr><th className="text-left p-3">File</th><th className="text-left p-3">By</th><th className="text-left p-3">When</th><th className="text-right p-3">Rows</th></tr></thead><tbody>{recentExports.map(function(e){ return <tr key={e.name} className="border-t border-gray-100 hover:bg-blue-50"><td className="p-3"><div className="font-medium text-gray-900 text-xs">{e.name}</div><div className="text-xs text-gray-500">{e.fmt}</div></td><td className="p-3 text-xs">{e.by}</td><td className="p-3 text-xs text-gray-500">{e.when}</td><td className="p-3 text-right text-xs">{e.rows}</td></tr>; })}</tbody></table></div></div>

    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden"><div className="p-4 border-b border-gray-200 flex items-center justify-between"><div className="font-bold text-gray-900">Scheduled &amp; Recurring</div><button className="text-xs bg-blue-600 text-white rounded-lg px-3 py-1.5 font-medium">+ Schedule</button></div><div className="divide-y divide-gray-100">{scheduledExports.map(function(s){ return <div key={s.name} className="p-4 flex items-center justify-between gap-3"><div><div className="font-medium text-gray-900 text-sm">{s.name}</div><div className="text-xs text-gray-500 mt-0.5">{s.cadence} · {s.fmt}</div><div className="text-xs text-gray-400">→ {s.to}</div></div><span className={'rounded-full px-2 py-1 text-xs font-medium ' + (s.on ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-500')}>{s.on ? 'Active' : 'Paused'}</span></div>; })}</div></div></div></div>;
  }
    ...
    async function doExcelExport(){
      ...
      if (groupBy !== 'none') {
        groups.forEach(function(g){
          var ws = wb.addWorksheet(g.label || 'Export');
          ws.columns = activeCols.map(function(c){ return { header: c.header, key: c.key, width: c.key === 'desc' || c.key === 'reason' ? 40 : c.key === 'recommendation' ? 30 : 18 }; });
          g.rows.forEach(function(r){ var row = {}; activeCols.forEach(function(c){ row[c.key] = (r[c.key] !== undefined && r[c.key] !== null) ? r[c.key] : ''; }); ws.addRow(row); });
          ws.getRow(1).eachCell(function(cell){ cell.font = { bold: true, name: 'Aptos Narrow', size: 11, color: { argb: 'FFFFFFFF' } }; cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF1F3864' } }; cell.alignment = { horizontal: 'center', vertical: 'middle' }; });
          ws.eachRow(function(row, rn){ if (rn === 1) return; row.eachCell(function(cell){ cell.font = { name: 'Aptos Narrow', size: 11 }; cell.alignment = { horizontal: 'center', vertical: 'middle' }; }); });
          if (selCols.indexOf('priority') >= 0) { ws.eachRow(function(row, rn){ if (rn === 1) return; var pCell = row.getCell('priority'); var col = PRIORITY_COLORS[pCell.value]; if (col) { pCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: col } }; pCell.font = { name: 'Aptos Narrow', size: 11, color: { argb: 'FFFFFFFF' } }; } }); }
          ws.views = [{ state: 'frozen', ySplit: 1 }];
          ws.autoFilter = { from: { row: 1, column: 1 }, to: { row: 1, column: activeCols.length } };
        });
      } else {
        var ws = wb.addWorksheet('Export');
        ws.columns = activeCols.map(function(c){ return { header: c.header, key: c.key, width: c.key === 'desc' || c.key === 'reason' ? 40 : c.key === 'recommendation' ? 30 : 18 }; });
        displayRows.forEach(function(r){ var row = {}; activeCols.forEach(function(c){ row[c.key] = (r[c.key] !== undefined && r[c.key] !== null) ? r[c.key] : ''; }); ws.addRow(row); });
        ws.getRow(1).eachCell(function(cell){ cell.font = { bold: true, name: 'Aptos Narrow', size: 11, color: { argb: 'FFFFFFFF' } }; cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF1F3864' } }; cell.alignment = { horizontal: 'center', vertical: 'middle' }; });
        ws.eachRow(function(row, rn){ if (rn === 1) return; row.eachCell(function(cell){ cell.font = { name: 'Aptos Narrow', size: 11 }; cell.alignment = { horizontal: 'center', vertical: 'middle' }; }); });
        if (selCols.indexOf('priority') >= 0) { ws.eachRow(function(row, rn){ if (rn === 1) return; var pCell = row.getCell('priority'); var col = PRIORITY_COLORS[pCell.value]; if (col) { pCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: col } }; pCell.font = { name: 'Aptos Narrow', size: 11, color: { argb: 'FFFFFFFF' } }; } }); }
        ws.views = [{ state: 'frozen', ySplit: 1 }];
      ...
      var url = URL.createObjectURL(blob); var a = document.createElement('a'); a.href = url; a.download = exportName + '.xlsx'; a.click(); URL.revokeObjectURL(url);
    }
