...
function ActionBadge(props) { return <span className={'px-2 py-1 rounded-full text-xs border font-medium ' + (actionTones[props.action] || 'bg-gray-100 text-gray-700 border-gray-200')}>{props.action}</span>; }
    ...
    return <div className="space-y-5"><div className="flex items-start justify-between gap-4"><div><h1 className="text-2xl font-bold text-gray-900">Audit History</h1><p className="text-gray-500">The immutable record of every change — imports, price/status edits, archives, flags, orders, rules, and role changes. Nothing is ever silently deleted; most actions are reversible from here.</p></div><button onClick={async function(){ var EXP_COLS = [{header:'ID',key:'id'},{header:'Timestamp',key:'ts'},{header:'User',key:'user'},{header:'Role',key:'role'},{header:'Action',key:'action'},{header:'Module',key:'module'},{header:'Target',key:'target'},{header:'Before',key:'before'},{header:'After',key:'after'},{header:'Reversible',key:'reversible'},{header:'Note',key:'note'}]; if (typeof ExcelJS === 'undefined') { alert('ExcelJS not loaded.'); return; } var wb = new ExcelJS.Workbook(); var ws = wb.addWorksheet('Audit Log'); ws.columns = EXP_COLS.map(function(c){ return { header: c.header, key: c.key, width: Math.max(c.header.length + 4, 20) }; }); auditLog.forEach(function(e){ var row = {}; EXP_COLS.forEach(function(c){ var v = e[c.key]; row[c.key] = (v === undefined || v === null) ? '' : (c.key === 'reversible' ? (v ? 'Yes' : 'No') : String(v)); }); ws.addRow(row); }); ws.getRow(1).eachCell(function(cell){ cell.font = { bold: true, name: 'Aptos Narrow', size: 11, color: { argb: 'FFFFFFFF' } }; cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF1F3864' } }; cell.alignment = { horizontal: 'center', vertical: 'middle' }; }); ws.eachRow(function(row, rn){ if (rn === 1) return; row.eachCell(function(cell){ cell.font = { name: 'Aptos Narrow', size: 11 }; cell.alignment = { horizontal: 'left', vertical: 'middle' }; }); }); ws.views = [{ state: 'frozen', ySplit: 1 }]; ws.autoFilter = { from: { row: 1, column: 1 }, to: { row: 1, column: EXP_COLS.length } }; var buf = await wb.xlsx.writeBuffer(); var blob = new Blob([buf], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' }); var url = URL.createObjectURL(blob); var a = document.createElement('a'); a.href = url; a.download = 'audit-log-' + Date.now() + '.xlsx'; a.click(); URL.revokeObjectURL(url); }} className="bg-blue-600 text-white rounded-lg px-4 py-2 text-sm font-medium hover:bg-blue-700">Export Log</button></div>
