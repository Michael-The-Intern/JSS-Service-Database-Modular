...
function Dashboard(props) {
  ...
  const { page, setPage, parts, rawParts, setRawParts, rawAudit, setRawAudit, _supaWrite,
    ...
    EVENT_TYPES, joysonLogoUrl, oemKeys
  } = ctx; 
...
function Section(props) {
      ...
      const visibleItems = expanded ? props.items : props.items.slice(0, PREVIEW_CAP);
      ...
      return <div className={'border-l-4 rounded-r-xl ' + (toneBar[props.tone] || toneBar.blue) + ' p-4'}><div className="flex items-center justify-between"><div className="font-bold text-gray-900">{props.icon} {props.title}</div><span className="bg-white border border-gray-200 rounded-full px-2 py-1 text-xs font-medium">{props.items.length} item(s)</span></div><p className="text-sm text-gray-600 mt-1">{props.blurb}</p><div className="mt-3 space-y-2">{visibleItems.map(function(it){ return <button key={it.id} onClick={props.onClick ? function(){ props.onClick(it); } : null} className="w-full text-left bg-white border border-gray-200 rounded-lg p-3 hover:bg-gray-50 flex items-center justify-between gap-3"><div><div className="text-sm font-semibold text-gray-900">{it.jss} <span className="font-normal text-gray-500">&middot; {it.oem}{it.plant ? ' · Plant ' + it.plant : ''}</span></div><div className="text-xs text-gray-500">{props.line ? props.line(it) : it.desc}</div></div><span className="text-gray-400">&rarr;</span></button>; })}</div>{props.items.length > PREVIEW_CAP && <button onClick={function(){ setExpanded(function(e){ return !e; }); }} className="mt-2 text-xs text-blue-600 hover:text-blue-800 font-medium underline underline-offset-2">{expanded ? 'Show preview' : 'View full part list (' + props.items.length + ' items)'}</button>}</div>;
    }
