...
function SignIn() {
  ...
  function _supaWrite(table, row) {
    var payload = (table === 'parts')
      ...
      : (table === 'audit_log')
        ...
        : (table === 'tasks')
          ...
          : row;
    // PATCH A (v14): return the Promise so callers can await and detect errors
    return _supa.from(table).upsert(payload).then(function(r){
      if (r && r.error) {
        ...
        return { error: r.error };  // propagate so Promise.allSettled sees failure
      }
      return { error: null, data: r.data };
    }, function(e){
      console.warn('[Supabase] network error — table:', table, e);
      throw e;  // reject so callers can catch network failures
    });
  }
  ...
  var ctxValue = {
    ...
    filteredParts: filteredParts, yearIsPinned: yearIsPinned, oemKeys: oemKeys, refData: refData,
